...
然后你将会看到 QQ 群配置、消息列表配置和 QQ 频道配置。根据你的需求来选择QQ群、允许私聊的QQ号、QQ频道。（‘系统维护中，非企业认证的开发者账号暂时无法修改‘群’的沙箱环境配置’3.20日，且官方更新提示‘如需开发在QQ群使用的机器人功能请完成此项配置暂不支持 AIGC 机器人进入社群场景以及上架后全量对所有用户使用，若有违反，平台会执行相关拦截策略，请开发者知悉。’）
